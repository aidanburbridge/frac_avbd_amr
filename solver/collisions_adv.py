...
def narrow_phase(pair: tuple[CollidableShape, CollidableShape]):
    ...
    for axis in all_axes:                                   # Project both bodies onto the current axis - get their shadows
        ...
        minA, maxA = project_on_axis(bodyA, unit_axis)           # minA and maxA are the corners projected onto the given axis
        minB, maxB = project_on_axis(bodyB, unit_axis)
    ...
    # Ensure A->B
    center_dir = bodyB.position[:2] - bodyA.position[:2]
    if np.dot(center_dir, collision_normal) < 0:
...
NO_EDGE, EDGE1, EDGE2, EDGE3, EDGE4 = 0,1,2,3,4
FACE_A_X, FACE_A_Y, FACE_B_X, FACE_B_Y = 0, 1, 2, 3
...
def _clipping(self, bodies:tuple[Body, Body, np.ndarray]):
    """
    Sutherland Hogman Polygon clipping algorithm.
    """

    bodyA, bodyB, collision_norm = bodies

    pass


# TODO for facture and grouped body collision
def ccd(self):
    """
    FEM style check of internal voxels.
    """
    pass